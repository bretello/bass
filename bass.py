...
def todayTheme(verbose=False):
    response = get(
        "https://www.bing.com/HPImageArchive.aspx?format=js&idx=0&n=1&mkt=it-IT"
    )
...
def renderErrPage(error):
    ...
    return env.get_template("error.html").render(
        ERROR=error, TITLE=error, COLOR=today_theme["COLOR"], BLOCK="error"
    )
...
def home():
    ...
    return env.get_template("home.html").render(
        TITLE="Welcome on BASS", COLOR=today_theme["COLOR"], BLOCK="home"
    )
...
def addAlbum():
    """Create new album"""
    return env.get_template("add_album.html").render(
        TITLE="Add album",
        COLOR=today_theme["COLOR"],
        BLOCK="add_album",
        NEW_HASH=newID(),
...
def addTracks():
    ...
    album = request.form
    return env.get_template("add_tracks.html").render(
        TITLE=f"Add tracks to {album['TITLE']}",
        ALBUM=album,
        LENGHT=int(album["LENGHT"]),
        COLOR=today_theme["COLOR"],
        BLOCK="add_tracks",
    )
...
def showAlbum(id):
    ...
    if album:
        return env.get_template("album.html").render(
            TITLE=f"{album['TITLE']} ({album['AUTHOR']})", ALBUM=album, BLOCK="album"
        )
    else:
